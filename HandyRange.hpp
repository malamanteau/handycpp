...
///
/// Generator for range-based "for" iteration. Very 
/// similar to Python's range(x, y, z) function. 
/// 
...
namespace HANDY_NS {
	...
	template <typename T> range_proxy         <T> range(T begin, T end) { return{ begin, end }; }
	...
	namespace traits {
		...
		struct has_size
		{
			template <typename T> static auto check(T*)  -> typename std::is_integral<decltype(std::declval<T const>().size())>::type;
	...
	auto indices(C const& cont) -> range_proxy<decltype(cont.size())> { return{ 0, cont.size() }; }

	template <typename T, std::size_t N>
	range_proxy<std::size_t> indices(T(&)[N]) { return{ 0, N }; }
