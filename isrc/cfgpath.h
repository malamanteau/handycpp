...
/// Copy of https://github.com/Malvineous/cfgpath
...
static inline void get_user_config_file(char *out, unsigned int maxlen, const char *appname)
{
	...
	*out = 0;
	...
	if (!SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, 0, (LPSTR)(void *)out))) {
		out[0] = 0;
		return;
	}
	/* We don't try to create the AppData folder as it always exists already */
	unsigned int appname_len = (unsigned int)strlen(appname);
...
static inline void get_user_cache_folder(char *out, unsigned int maxlen, const char *appname)
{
	...
	*out = 0;
	...
	if (!SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, (LPSTR)(void *)out))) {
		out[0] = 0;
		return;
	}
	/* We don't try to create the AppData folder as it always exists already */
