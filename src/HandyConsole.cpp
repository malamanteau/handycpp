...
namespace HANDY_NS::Console
{
	...
	public:
		...
		std::string RemoveNonASCII(std::string const & s)
		{
			std::string ret;
			for (char c : s)
				if ((c >= ' ' && c <= '~') || c == '\r' || c == '\n' || c == '\t')
		...
		void log(std::string const & text)
		{
			...
			for (std::vector<std::string>::iterator it = lines.begin(), end = lines.end(); it != end; ++it)
			{
				...
				{
					std::stringstream ssConsole, ssFunction;
					...
					if (Function)
					{
						//std::string cleaned = RemoveNonASCII(ssFunction.str());

						Function(ssFunction.str()); //cleaned);
